...
(86M params, injection-only, Groq) to LlamaGuard 3 8B (8B params, 13 safety
categories, Ollama/Together AI).

Key changes from S14:
  - _llamaguard_safe now returns bool (not tuple[bool, float])
  - guard returns {"blocked_reason": ...} without llamaguard_score
  - S6 (Specialized Advice) is excluded from blocking — COMPLEX router handles it
  - LlamaGuard backend is Ollama (default) or Together AI
...
def _llamaguard_safe(message: str) -> bool:
    ...
    loan?") trigger S6. But QuickLoan's COMPLEX routing already handles these
    correctly by escalating to a loan officer. Blocking at the guard gives a
    worse user experience — so S6-only results are passed through.
    ...
    try:
        ...
        safe = len(non_s6) == 0
        print(f"[QuickLoan] LlamaGuard: {verdict!r} → {'safe (S6 handled by router)' if safe else 'UNSAFE'}")
        return safe
    except Exception as e:
...
def guard(state: QuickLoanState) -> dict:
    ...
    # Layer 2: LlamaGuard 3 8B — broad content safety classification.
    if not _llamaguard_safe(msg):
        print("[QuickLoan] Guard: LlamaGuard flagged message — blocked")
